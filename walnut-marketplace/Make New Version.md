...
- **Where to run the CLI:** depends how the Vercel project’s **Root Directory** is set (Project → Settings → General).
  - **Root Directory = `walnut-marketplace` (subfolder):** run `npx vercel deploy` from the **Git repository root** — the folder that contains the `walnut-marketplace` directory (e.g. `d:\Project Walnut`), **not** from inside `walnut-marketplace`. Otherwise the CLI can resolve a non-existent path like `…\walnut-marketplace\walnut-marketplace`.
  - **Root Directory = `./` (app at repo root):** run the CLI from the same folder as `package.json` (the repo root).
- **`.vercel` link:** after `vercel link`, `.vercel/project.json` is gitignored. It must live in the directory **from which you invoke** `vercel deploy`. If you deploy from the Git root but linked only inside `walnut-marketplace`, copy the same `.vercel/project.json` to the Git root (or run `vercel link` once from that root).
...
1. **Production deploy (CLI)** — `cd` to the correct directory (see Prerequisites), then:
   ...
   **Windows PowerShell** (example when Git root is `Project Walnut` and Root Directory is `walnut-marketplace`):

   ```powershell
   Set-Location "d:\Project Walnut"
   npx vercel deploy --prod --yes
   ```
...
The CLI prints a deployment URL; production is usually aliased to your project’s primary `.vercel.app` domain (e.g. `https://walnut-marketplace.vercel.app`). Inspect a deployment with `npx vercel inspect <deployment-url>`.
...
- **`…\walnut-marketplace\walnut-marketplace` does not exist:** Your shell was probably inside the inner app folder while Vercel **Root Directory** is set to `walnut-marketplace`. **cd** to the **Git root** and deploy again (see Prerequisites).
