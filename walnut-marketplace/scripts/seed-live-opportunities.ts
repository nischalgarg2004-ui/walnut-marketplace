/**
 * Seeds the live "Walnut" demo business: loginable account, 5 published requirements,
 ...
 *   Email: walnut.postings@walnut.demo
 *   Password: WalnutDemo2026!
 *
 ...
 *   PowerShell: $env:WALNUT_ENV_FILE=".env.production.vercel"; npm run seed:opportunities
 */
...
function loadDatabaseUrlFromEnvFiles() {
  const explicit = process.env.WALNUT_ENV_FILE;
  if (explicit) {
...
const WALNUT_BUSINESS_PASSWORD = "WalnutDemo2026!";

const SEED_BUSINESS_EMAIL = "walnut.postings@walnut.demo";
const LEGACY_SEED_EMAIL = "live-feed-seed@walnut.local";
const TITLE_PREFIX = "Walnut:";
...
async function main() {
  const passwordHash = hashSync(WALNUT_BUSINESS_PASSWORD, 10);
  ...
  const profile = await prisma.businessProfile.upsert({
    ...
    update: {
      legalName: "Walnut Pvt Ltd (Demo)",
      brandName: "Walnut",
      category: "Marketplace demo",
      website: "https://walnut-marketplace.vercel.app",
  ...
  await prisma.requirement.deleteMany({
    where: {
      ...
      OR: [{ title: { startsWith: TITLE_PREFIX } }, { title: { startsWith: "Walnut Demo:" } }]
    }
  });
  ...
  console.log("Walnut live seed complete.", {
    loginEmail: SEED_BUSINESS_EMAIL,
    loginPassword: WALNUT_BUSINESS_PASSWORD,
