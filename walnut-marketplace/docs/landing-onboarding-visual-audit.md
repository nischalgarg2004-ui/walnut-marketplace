...
- **Major**: Hero conversion path was split across multiple actions with weak hierarchy.  
  **Resolution**: Replaced with one primary CTA (`Enter OnGram`) and one trust-secondary action.
...
- Flow wording remains consistent with `Enter OnGram` and clear branch language.
