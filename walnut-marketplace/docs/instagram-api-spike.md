...
- Reel metrics are sourced from Graph API only; when Graph is unavailable, Walnut keeps last-known stored metrics as fallback.

### Live-domain creator insights validation

- Walnut validates Instagram Login capability directly in creator production surfaces:
  - `GET/POST /api/creator/insights`
  - `/creator/profile` -> `Instagram insights` section
  - `/creator/deals/[contractId]` -> campaign-linked views/diagnostics block
...
- Walnut now requests insights using a media-type-aware metric policy:
...
2. Confirm runtime env on deployment:
   ...
   - `INSTAGRAM_SCOPE`
3. Open `/creator/profile` and run `Refresh insights` with a connected creator profile.
