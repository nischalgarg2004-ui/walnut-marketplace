...
// Paths
const srcUserDir = path.join('C:', 'Users', 'nisch', 'AppData', 'Local', 'Google', 'Chrome', 'User Data');
const destUserDir = path.join('C:', 'Users', 'nisch', '.gemini', 'antigravity', 'scratch', 'temp_profile');
...
console.log('=== Copying Chrome Profile Essentials ===');
// Copy Local State (contains the encrypted key for cookies)
copyFileSafe(path.join(srcUserDir, 'Local State'), path.join(destUserDir, 'Local State'));
// Copy Cookies database
copyFileSafe(path.join(srcUserDir, 'Default', 'Network', 'Cookies'), path.join(destUserDir, 'Default', 'Network', 'Cookies'));

(async () => {
  console.log('\n=== Launching Puppeteer with temporary profile ===');
  ...
  try {
    ...
    // Take screenshot of editor to verify login status
    const screenshotPath = path.join(__dirname, 'temp_editor_view.png');
    ...
    if (!hasLogin) {
      console.log('Successfully logged in using Chrome cookies!');
      
      // Navigate to the prototype URL to force the video to load and stream
      ...
      const protoScreenshotPath = path.join(__dirname, 'temp_proto_view.png');
      await page.screenshot({ path: protoScreenshotPath });
      console.log(`Saved prototype view screenshot to ${protoScreenshotPath}`);
    } else {
      console.log('Cookies did not authenticate the session. Still seeing login screen.');
    }
