...
const destUserDir = path.join('C:', 'Users', 'nisch', '.gemini', 'antigravity', 'scratch', 'temp_chrome_profile');
...
copyFileSafe(path.join(chromeUserDir, 'Local State'), path.join(destUserDir, 'Local State'));
copyFileSafe(path.join(chromeUserDir, 'Default', 'Network', 'Cookies'), path.join(destUserDir, 'Default', 'Network', 'Cookies'));

(async () => {
  console.log('\n=== Launching Puppeteer with temporary Chrome profile ===');
  ...
  try {
    ...
    if (!hasLogin) {
      ...
      console.log('Chrome cookies did not authenticate the session.');
    }
