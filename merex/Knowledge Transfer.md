...
Authenticated **app sections** (`/creator/*`, `/business/*`, `/admin/*`) pass through [`src/middleware.ts`](./src/middleware.ts): it checks for the `merex_session` cookie and redirects to `/login?next=…` if missing. Full session verification and **role** checks happen in route handlers and layouts, not only in middleware.
...
The **Git repository** may have the Next.js app in a subfolder named `merex/` (repo root one level above `package.json`). Vercel **Root Directory** is often set to that subfolder—see [Make New Version.md](./Make%20New%20Version.md) for where to run `vercel deploy` from.
