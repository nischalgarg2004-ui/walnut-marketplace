# Merex - India Creator Marketplace MVP

Lean MVP for a two-sided marketplace where brands post creator requirements and creators apply based on eligibility.

## Implemented Scope

- Auth + RBAC: HTTP-only session cookie (`merex_session`, signed with `SESSION_SECRET`), login at `/login`
...
Merex is now structured to support a preview-first Vercel workflow:
