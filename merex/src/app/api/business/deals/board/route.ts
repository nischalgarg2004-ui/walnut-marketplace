  ...
  try {
    ...
    const contracts = await db.contract.findMany({
      ...
      include: {
        ...
        requirement: {
          select: { title: true, id: true, category: true, deliveryDueAt: true, deliveryDueOffsetDays: true }
        },
        application: { select: { status: true, appliedAt: true } },
        deliverables: { orderBy: { submittedAt: "desc" }, include: { submissions: { orderBy: { submittedAt: "desc" } } } },
        barterShipment: true,
    ...
    const rows = contracts.map((c) => ({
      ...
      latestDeliverable: c.deliverables[0]
        ? {
            ...
            latestSubmissionStage: c.deliverables[0].submissions[0]?.stage ?? null,
            latestSubmissionStatus: c.deliverables[0].submissions[0]?.status ?? null
          }
        : null,
      nextAction: (() => {
        const all = c.deliverables;
        const isClipping = c.requirement.category === "CLIPPING";
        const waitingDraft = all.some((d) =>
          !d.submissions.some((s) => s.stage === "DRAFT" && s.status === "APPROVED")
        );
        const waitingPublish = all.some((d) =>
          !d.submissions.some((s) => s.stage === "PUBLISHED_LINK" && s.status === "APPROVED")
        );
        if (!isClipping && waitingDraft) return "Review draft submissions";
        if (waitingPublish) return "Review publish links";
        if (c.status !== "COMPLETED") return "Mark contract complete / trigger payout";
        return "Completed";
      })(),
