...
function isRecoverableRefreshFailure(message: string): boolean {
  const lower = message.toLowerCase();
  return lower.includes("unsupported request - method type: get") || lower.includes("code=100");
}
...
async function runInsights(req: NextRequest) {
  ...
  if (profile.instagramTokenExpiresAt && shouldRefreshInstagramToken(profile.instagramTokenExpiresAt)) {
    try {
      ...
      const msg = err instanceof Error ? err.message : "Failed to refresh Instagram token";
      if (isRecoverableRefreshFailure(msg)) {
        console.warn("[creator/insights] refresh rejected by Instagram; continuing with current token", {
  ...
  const storedUsername = (profile.instagramUsername ?? profile.instagramHandle ?? "").replace(/^@/, "").trim();
  const identity = profile.instagramUserId
    ? {
        userId: profile.instagramUserId,
        username: storedUsername || `instagram_${profile.instagramUserId}`,
        accountType: profile.instagramAccountType === "BUSINESS" ? "BUSINESS" : "CREATOR",
        accessToken
      }
    : null;
  if (!identity) {
