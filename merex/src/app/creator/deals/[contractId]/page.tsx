...
export default function CreatorDealDetailPage() {
  ...
  async function submitSlot(deliverableId: string) {
    if (!data?.contract || data.contract.status !== "ACTIVE") {
      setToast({ message: "Accept the contract first to submit deliverables.", type: "error" });
      return;
    }
    ...
    const deliverable = data?.contract?.deliverables.find((d) => d.id === deliverableId);
    const draftApproved = Boolean(
      deliverable?.submissions?.find((s) => s.stage === "DRAFT" && s.status === "APPROVED")
    );
    const stage = draftApproved ? "PUBLISHED_LINK" : "DRAFT";
    const res = await fetch(`/api/deliverables/${deliverableId}/submissions`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ stage, url, fileType: "link" })
    });
    const json = await res.json();
    ...
    setToast({
      message:
        stage === "DRAFT"
          ? "Draft submitted for business review."
          : "Publish link submitted for business verification.",
      type: "success"
    });
    await loadDeal();
  }

  async function acceptContractFromDetail() {
    if (!data?.contract) return;
    const res = await fetch(`/api/creator/contracts/${data.contract.id}/accept`, { method: "POST" });
    const json = await res.json();
    if (!res.ok) {
      setToast({ message: json.error ?? "Could not accept contract", type: "error" });
      return;
    }
    setToast({ message: "Contract accepted. Deliverables are now open for submission.", type: "success" });
    await loadDeal();
  }
      ...
      {c && c.status === "ACTIVE" && (
        <div className="card">
          ...
          <p className="muted text-sm mb-4">
            Paste the public Instagram reel (or post) URL for this campaign. Merex calls Instagram Graph
            {" "}
            <code>GET /&#123;media-id&#125;/insights</code> on <code>graph.instagram.com</code> from your
            connected creator account (uses <code>instagram_business_manage_insights</code>) to record verified
            view counts. Used as a basis for view-based payouts when your brand uses CPV.
          </p>
          <form onSubmit={(e) => void saveReelLink(e)} className="stack" style={{ gap: "0.75rem" }}>
            ...
            <div className="row" style={{ flexWrap: "wrap", gap: "0.5rem" }}>
              <button
                ...
                type="submit"
                disabled={saving}
                title="Resolves the Instagram media id from your Reel URL and calls Graph /{media-id}/insights to refresh views (uses instagram_business_manage_insights)."
              >
              ...
              <button
                ...
                title="Calls Instagram Graph /{media-id}/insights for the saved Reel (uses instagram_business_manage_insights)."
              >
        ...
        <div className="card">
          ...
          {c.status === "PENDING" ? (
            <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">
              <p className="m-0">Accept this contract to unlock deliverable-wise upload for review.</p>
              <button className="btn secondary mt-2" onClick={() => void acceptContractFromDetail()}>
                Accept contract
              </button>
            </div>
          ) : null}
              ...
              <article key={d.id} className="rounded-lg border border-border p-3">
                {(() => {
                  ...
                  const canSubmit = Boolean(
                    !draft || draft.status === "REVISION_REQUESTED" || draft.status === "SUBMITTED"
                      ? true
                      : draft.status === "APPROVED"
                  );
                  const isDraftPhase =
                    !draft || draft.status === "SUBMITTED" || draft.status === "REVISION_REQUESTED";
                  return (
                    <div className="mb-2 rounded border border-border/70 p-2 text-xs">
                      <p className="m-0">
                        Draft: <strong>{draft ? draft.status : "NOT_SUBMITTED"}</strong>
                      </p>
                      <p className="m-0 mt-1">
                        Publish link: <strong>{publish ? publish.status : "NOT_SUBMITTED"}</strong>
                      </p>
                      {!canSubmit ? <p className="m-0 mt-1 text-amber-700">Waiting for draft approval.</p> : null}
                      <p className="m-0 mt-1 text-muted-foreground">
                        Next action: {isDraftPhase ? "Submit draft asset" : "Submit Instagram publish link"}
                      </p>
                    </div>
                  );
                })()}
                ...
                {(d.status === "PENDING" ||
                  ...
                  <div className="mt-2 flex flex-col gap-2">
                    ...
                    <button
                      ...
                      disabled={submittingSlotId === d.id || c.status !== "ACTIVE"}
                      onClick={() => void submitSlot(d.id)}
                    >
                      {submittingSlotId === d.id
                        ? "Submitting..."
                        : c.status !== "ACTIVE"
                          ? "Accept contract to submit"
                          : "Submit deliverable"}
                    </button>
                  </div>
                )}
