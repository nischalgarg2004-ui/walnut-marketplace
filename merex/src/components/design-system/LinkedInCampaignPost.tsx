...
export function LinkedInCampaignPost({ post }: { post: LinkedInCampaignPostData }) {
  ...
  function CommentIcon() {
    return (
      <svg viewBox="0 0 24 24" className="h-5 w-5" aria-hidden>
        <path d="M4 5.5A2.5 2.5 0 0 1 6.5 3h11A2.5 2.5 0 0 1 20 5.5v8A2.5 2.5 0 0 1 17.5 16H10l-5 4v-4H6.5A2.5 2.5 0 0 1 4 13.5v-8Z" fill="none" stroke="currentColor" strokeWidth="1.8" />
        <path d="M8 8.5h8M8 11.5h8" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" />
      </svg>
    );
  }
  ...
  function ShareIcon() {
    return (
      <svg viewBox="0 0 24 24" className="h-5 w-5" aria-hidden>
        <path d="m20 4-9 9" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" />
        <path d="M20 4h-6M20 4v6" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" />
        <path d="M4 12v5a3 3 0 0 0 3 3h10" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" />
      </svg>
    );
  }
  ...
  function ApplyIcon() {
    return (
      <svg viewBox="0 0 24 24" className="h-5 w-5" aria-hidden>
        <path d="M7 12.5 10.2 16 17 8.8" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        <rect x="3.5" y="3.5" width="17" height="17" rx="3.5" fill="none" stroke="currentColor" strokeWidth="1.5" />
      </svg>
    );
  }
  ...
  return (
    <article className="relative overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
      ...
      <div className="grid grid-cols-4 border-t border-border/70">
        <button
          type="button"
          className="inline-flex min-h-[52px] items-center justify-center border-r border-border/60 text-muted-foreground transition hover:bg-muted/30 hover:text-foreground"
          onClick={() => void toggleReaction()}
          disabled={busy !== null}
          aria-label={viewerReaction === "LIKE" ? "Unlike post" : "Like post"}
          title={viewerReaction === "LIKE" ? "Liked" : "Like"}
        >
          {/* eslint-disable-next-line @next/next/no-img-element -- local UI icon asset from public/icons */}
          <img
            src={viewerReaction === "LIKE" ? "/icons/like-outline.png" : "/icons/like-filled.png"}
            alt=""
        ...
        <Link
          href={post.applyHref as Route}
          className="inline-flex min-h-[52px] items-center justify-center text-muted-foreground transition hover:bg-muted/30 hover:text-foreground"
          aria-label={post.applyLabel}
          title={post.applyLabel}
        >
          <ApplyIcon />
        </Link>
      </div>
