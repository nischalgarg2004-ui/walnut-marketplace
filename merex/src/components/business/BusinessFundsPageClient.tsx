...
import { PAYMENT_DISPLAY_NAME } from "@/lib/brand";
  ...
  async function addFunds(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!key) {
        let diagnostic = "";
        try {
          const debugRes = await fetch("/api/debug/razorpay-config");
          const debugJson = await debugRes.json();
          if (debugRes.ok) {
            diagnostic = ` [diag: nodeEnv=${debugJson.data?.nodeEnv ?? "unknown"}, hasNextPublic=${String(
              debugJson.data?.hasNextPublicRazorpayKey
            )}, hasServerKey=${String(debugJson.data?.hasServerRazorpayKey)}, hasServerSecret=${String(
              debugJson.data?.hasServerRazorpaySecret
            )}]`;
          } else {
            diagnostic = ` [diag: debug endpoint ${debugRes.status}]`;
          }
        } catch {
          diagnostic = " [diag: debug fetch failed]";
        }
        console.error("Razorpay config missing on client bundle", {
          host: window.location.host,
          href: window.location.href,
          nodeEnv: process.env.NODE_ENV
        });
        setError(
          "Missing NEXT_PUBLIC_RAZORPAY_KEY_ID in client bundle. If local: set in .env.local and restart `npm run dev`. If deployed: set NEXT_PUBLIC_RAZORPAY_KEY_ID in Vercel project env and redeploy." +
            diagnostic
        );
        return;
      }
      ...
      const amountPaise = Math.max(0, Math.round(Number(amount) * 100));
      if (amountPaise < 100) {
        setError("Minimum top-up is ₹1 (100 paise).");
        return;
      }
      ...
      const orderId = json.data?.order_id as string | undefined;
      const orderAmount = json.data?.amount as number | undefined;
      const currency = (json.data?.currency as string | undefined) ?? "INR";
